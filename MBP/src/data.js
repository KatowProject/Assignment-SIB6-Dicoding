...
const getBooks = (query) => {
    if (query.name) {
        ...
        return books.filter(({ reading: bookReading }) => bookReading === reading).map(({ id, name, publisher }) => ({ id, name, publisher }));
    } else if (query.finished) {
        const finished = query.finished === '1';
        return books.filter(({ pageCount, readPage }) => (pageCount === readPage) === finished).map(({ id, name, publisher }) => ({ id, name, publisher }));
    }
